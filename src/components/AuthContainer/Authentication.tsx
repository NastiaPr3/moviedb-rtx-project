...
const Authentication = () => {
    ...
    const {req, accountId, sessionId, movies, deleteSes} = useAppSelector(state => state.auth);
    ...
    if (requestToken) {
        window.location.href = `https://www.themoviedb.org/authenticate/${requestToken}?redirect_to=http://localhost:3002/auth`;
    ...
    const deleteSess = deleteSes && deleteSes;
